...
async function fixN8NPostgresMinimal() {
  console.log('🔧 Fixing N8N Postgres Store Interaction Query (Minimal)...\n');
  ...
  try {
    ...
    // Try minimal update
    console.log('📤 Updating workflow with minimal payload...');
    ...
    // Try alternative method - just update the specific node
    console.log('\n🔄 Trying alternative method - direct node update...');
    try {
      const nodeUpdateResponse = await axios.post(
        `${N8N_URL}/api/v1/workflows/WP5aiR5vN2A9w91i/nodes`,
        {
          nodeId: postgresNode.id,
          parameters: {
            ...postgresNode.parameters,
            query: fixedQuery
          }
        },
        {
      ...
      console.log('✅ Node updated via alternative method!');
    } catch (altError) {
