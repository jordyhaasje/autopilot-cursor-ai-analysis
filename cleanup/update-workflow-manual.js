...
async function updateWorkflowManual() {
  console.log('🔧 Updating Workflow Manually...\n');

  try {
    // Read current workflow
    console.log('📖 Reading current workflow...');
    const workflowData = JSON.parse(fs.readFileSync('workflow-current.json', 'utf8'));
    console.log(`✅ Workflow loaded: ${workflowData.nodes.length} nodes`);

    // Find Email Filter node
    const emailFilterNode = workflowData.nodes.find(node => node.name === 'Email Filter');
    if (!emailFilterNode) {
...
return await filterEmail();`;
    ...
    fs.writeFileSync('workflow-updated.json', JSON.stringify(workflowData, null, 2));
    console.log('✅ Updated workflow saved to workflow-updated.json');

    // Try to update using PUT with minimal payload
