...
async function fixCompensationLogicMinimal() {
  console.log('🔧 Fixing Compensation Logic (Minimal)...\n');
...
html = html.replace(/ich verstehe, dass die \\d+% entschädigung nicht ausreicht/gi, 'ich verstehe, dass sie mehr entschädigung möchten');`;
  ...
  if (finalOffer <= lastKnown) {
    ...
    // Update the workflow with minimal required fields
    console.log('\n📤 Updating workflow...');
    await axios.put(
      `${N8N_URL}/api/v1/workflows/WP5aiR5vN2A9w91i`,
      {
    ...
    // If the update fails, provide manual instructions
    console.log('\n📝 MANUAL FIX INSTRUCTIONS:');
    console.log('Since the automatic update failed, you need to manually update the workflow:');
    console.log('');
    console.log('1. Go to: https://primary-production-9667.up.railway.app/workflow/WP5aiR5vN2A9w91i');
    console.log('2. Edit the "Prompt Generator" node and add these lines to the SYSTEM prompt:');
    console.log('   - CRITICAL: When customer refuses an offer, offer the NEXT ladder step. Do NOT say the current offer is insufficient.');
    console.log('   - CRITICAL: If customer refuses 15%, offer 20%. If customer refuses 20%, offer 30%. Never repeat the same percentage.');
    console.log('   - CRITICAL: Do not mention that the current offer is "not sufficient" or "not enough" - just offer the next step.');
    console.log('');
    console.log('3. Edit the "Offer Normalizer" node and add this code before the final return:');
    console.log('   // CRITICAL FIX: Remove any mention of current offer being insufficient');
    console.log('   html = html.replace(/ik begrijp dat de \\d+% compensatie niet voldoende is/gi, "ik begrijp dat je meer compensatie wilt");');
    console.log('   html = html.replace(/i understand that the \\d+% compensation is not sufficient/gi, "i understand you want more compensation");');
    console.log('');
    console.log('4. Save the workflow and test with a new email.');
  }
}

fixCompensationLogicMinimal();
